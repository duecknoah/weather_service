...
app.get('/api/weather/hourly', (req, res) => {
  ...
  getHourlyWeather(city).then(json => {
    return res.send(json);
  });
});

/**
 * Makes an HTTP GET request to the openweathermap API to get the current weather information as a JSON.
 *
...
function getCurrentWeather(city) {
  return fetch(`http://api.openweathermap.org/data/2.5/weather?q=${city}&appid=${CONF_DATA.API_KEY}`)
    .then(res => res.json())
    .then(json => {
      // the incoming json response has more data than we need: See https://openweathermap.org/current#JSON
      // We need to extract what we want from the json and return the new modified JSON as the response.
      let newJSON = {
        "cod": json.cod
      };
      if (json.cod != "200") {
...
/**
 * Makes an HTTP GET request to the openweathermap API to get every 3 hours of weather information as a JSON.
 *
 * @param {String} city The name of the city to fetch current weather.
 * @returns {Promise} JSON info of the current weather
 */
function getHourlyWeather(city) {
  ...
  return fetch(`http://api.openweathermap.org/data/2.5/forecast?q=${city}&cnt=${CNT_LIMIT}&appid=${CONF_DATA.API_KEY}`)
    .then(res => res.json())
    .then(json => {
      // the incoming json response has more data than we need: See https://openweathermap.org/forecast5#JSON
      // We need to extract what we want from the json and return the new modified JSON as the response.
      let newJSON = {
        "cod": json.cod
      };
